...
2. **darkfix patches** under `ds1-patch/` and `ds2-patch/`. darkfix-ds1 0.1.1 and darkfix-ds2 0.1.0 ship the dead-trigger fixes (`fix.ds1.deadtriggers`, `fix.ds2.deadtriggers`). Applied to a player's GOG install; the game still launches via DOSBox under the original engine.
...
- **Hand-curated TOML catalogues** live alongside the tool that consumes them: `tools/gpl-disasm/syms/{opcodes,functions,variables,locals}.toml`, `tools/ovr-map/syms/{ds1,ds2}.toml`, `tools/save-inspect/syms/{items,save-fields}.toml`, `tools/dialog-extract/syms/speakers.toml`, and `docs/source-hashes/{ds1,ds2}-gog-1.10.toml`.
...
3. ⚠ **Ghidra does not understand Borland overlays.** A plain import leaves the overlay area as undifferentiated bytes, which is the blob problem the docs already work around by hand. `ovr-map` (Phase 5.5) is the bridge, and that pairing is the actual reason to install Ghidra. It maps 935 (DS1) / 854 (DS2) confirmed function entries.
