...
2. **darkfix patches** under `ds1-patch/` and `ds2-patch/`. darkfix-ds1 0.1.0 ships the first fix (`fix.ds1.deadtriggers`); ds2-patch is still forthcoming. Applied to a player's GOG install; the game still launches via DOSBox under the original engine.
