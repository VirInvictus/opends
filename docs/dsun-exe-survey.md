...
7. **The official-patch diff is blocked, and the roadmap's
   assumption behind it is wrong.** The floppy 1.0 binary is a
   different *build*, not an earlier revision of the CD build:
   resident image +1,840 bytes, 866 vs 854 entry points, and
   42 of 49 overlay segments differ materially when paired by
   index (section 8). Diffing needs a CD 1.0 base, which no
   archive artifact provides.
...
## 8. The official-patch diff: blocked, with evidence

The roadmap hoped to localize SSI's own 1.0-to-1.10 fixes by
diffing binaries. Measured result:

| | floppy 1.0 | GOG 1.10 |
|---|---|---|
| size | 634,208 | 634,416 |
| resident image end | `0x56e30` | `0x57570` (+1,840 B) |
| entry points | 866 | 854 |
| sha256 (prefix) | `bc9cdcbd...` | `ce02ee1f...` |

The floppy 1.0 binary found in the archive
(`ds2-floppy-1.0/DSUN.EXE`, byte-identical to the DSUN.EXE in
`dsun2_english.zip`) is a *different build*, not a near
revision: pairing overlay segments by index, 42 of 49 differ
materially (many by 30-90% of their bytes), sizes shift
throughout, and 6 segments (1, 36, 45, 46, 47, 48) are
byte-identical. A raw byte diff is 74% different and
meaningless.

`RESFLOP.GFF` (section 6) independently shows the floppy line
diverges at the source level. Conclusion: **localizing SSI's
1.02/1.10 fixes by diffing requires the CD 1.0 `DSUN.EXE`**, i.e.
a pre-patch CD-tree artifact (the `cd11` archive holds the
RTPatch package itself, `PATCH.RTP` + `PATCH.EXE`, not a base
tree; the RTPatch format could in principle be parsed for old
fingerprints, but that is its own project). No DS1 1.0
artifact exists in the archive at all.
