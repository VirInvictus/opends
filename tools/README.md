...
| [`gpl-disasm`](gpl-disasm/) | Rust | 0.7.0 | GPL bytecode disassembler: text or JSON, CFG labels, curated symbol catalogues (functions, variables, per-chunk locals), inter-chunk callgraph (`--global-cfg`), lossless packed-string decode. |
...
| [`ovr-map`](ovr-map/) | Python | 0.3.2 | Maps `DSUN.EXE`'s Borland overlay structure: segments, entry stubs, 16-bit disassembly, curated symbol catalogue (`syms/<game>.toml`), Ghidra bridges, and the string-xref tooling under `scripts/`. |
