...
| [`save-inspect`](save-inspect/)          | Python | 0.4.0   | Dump a CHARSAVE.GFF as JSON or diff two of them. v0.2 walked the CHAR body into combat / character / item sub-blocks for DS1 full schema; v0.3 adds DS2 combat partial decode (DS1-shared prefix + heuristic stats) and a `diff` subcommand. **v0.4.0 locks the full 49-byte DS2 combat layout** (corpus-validated on every CHAR in the GOG 1.10 save) with first-class `stats` + `name` fields, replacing the heuristic `_likely_*` keys. DS2 character sub-block (66 bytes) still queued for v0.5.0.|
