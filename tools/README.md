...
| [`gpl-asm`](gpl-asm/)                    | Rust   | 0.3.0   | GPL bytecode reassembler. Consumes `gpl-disasm --json` (600/600 byte-identical) or the text listing in either `--no-labels` or labelled form (both 600/600). v0.3.0 adds an `Editor` API for structural inserts / deletes / replaces with automatic branch retargeting. The other half of the GPL loop with `gpl-disasm`.|
