...
| [`save-inspect`](save-inspect/)          | Python | 0.5.0   | Dump a CHARSAVE.GFF as JSON or diff two of them. v0.2 walked the CHAR body into combat / character / item sub-blocks for DS1 full schema; v0.3 adds DS2 combat partial decode (DS1-shared prefix + heuristic stats) and a `diff` subcommand. v0.4.0 locked the full 49-byte DS2 combat layout. **v0.5.0** closes the DS2 character (66 bytes): full `current_xp`, `high_xp`, `base_hp`, `high_hp`, `base_psp`, `id`, `alignment`, `stats`, `real_class`, `level`, AC / movement, saving throws, sound / palette fields. DS2 layout is DS1's 72-byte minus 6 (drops `_data2` and two of race/gender/alignment). DS2 item sub-blocks still queued for v0.6.0.|
...
| [`repro`](repro/)                        | Shell + Python | 0.2.0 | DOSBox-Staging repro harness. Boots a per-bug fixture under `bugs/<id>/` with overlay-mounted writes (the install stays byte-identical), evaluates pass/fail by elapsed time + scratch-dir artifacts. v0.1.0 shipped the harness pattern + `ds1-smoke`. **v0.2.0** adds `ds2-smoke` (DS2 boots through `imgmount` of `game.ins` CD audio), captures DOSBox stderr to `<scratch>/dosbox.log`, adds `--list` for fixture enumeration, surfaces a DSUN.LOG preview on early-exit FAIL, and ships a `bugs/README.md` catalogue. Input automation and video capture roll into v0.3.0.|
