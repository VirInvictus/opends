...
| [`gpl-asm`](gpl-asm/)                    | Rust   | 0.2.1   | GPL bytecode reassembler. Consumes `gpl-disasm --json` (600/600 byte-identical) or the text listing — `--no-labels` or labelled (both forms 600/600 byte-identical). Auto-detects input format from extension. The other half of the GPL loop with `gpl-disasm`.|
