...
| [`gpl-asm`](gpl-asm/)                    | Rust   | 0.2.0   | GPL bytecode reassembler. Consumes `gpl-disasm --json` (600/600 byte-identical) or `gpl-disasm --no-labels` text (456/456 non-Search round-trip). Auto-detects input format from extension; the other half of the GPL loop with `gpl-disasm`.|
