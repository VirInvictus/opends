...
| [`gpl-disasm`](gpl-disasm/)              | Rust   | 0.4.3   | GPL bytecode disassembler. 100% corpus alignment on DS1+DS2 GPL/MAS. Recursive-descent CFG with labeled branches (v0.3.x), hand-curated symbol catalogue for function-entry and opcode-mnemonic decoration (v0.4.0 / v0.4.2), a whole-file inter-chunk callgraph via `--global-cfg` (v0.4.1), and a lossless 7-bit packed-string decoder unblocking the upcoming `gpl-asm` round-trip reassembler (v0.4.3). `--json` mode for downstream tools.|
