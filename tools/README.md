...
| [`gpl-disasm`](gpl-disasm/)              | Rust   | 0.4.4   | GPL bytecode disassembler. 100% corpus alignment on DS1+DS2 GPL/MAS. Recursive-descent CFG with labeled branches (v0.3.x), hand-curated symbol catalogue for function-entry and opcode-mnemonic decoration (v0.4.0 / v0.4.2), a whole-file inter-chunk callgraph via `--global-cfg` (v0.4.1), lossless 7-bit packed-string decoder (v0.4.3), and `Deserialize` impls on every public type so `gpl-asm` can re-encode the same JSON (v0.4.4). `--json` mode for downstream tools.|
| [`gpl-asm`](gpl-asm/)                    | Rust   | 0.1.0   | GPL bytecode round-trip reassembler. Consumes `gpl-disasm --json` and emits byte-identical bytecode. 456/600 DS1+DS2 GPLDATA chunks round-trip cleanly; 144 skipped pending `gpl_search` side-byte preservation (v0.1.x). The other half of the GPL loop with `gpl-disasm`.|
