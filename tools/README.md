...
| [`gpl-disasm`](gpl-disasm/)              | Rust   | 0.4.2   | GPL bytecode disassembler. 100% corpus alignment on DS1+DS2 GPL/MAS. Recursive-descent CFG with labeled branches (v0.3.x), hand-curated symbol catalogue for function-entry and opcode-mnemonic decoration (v0.4.0 / v0.4.2), and a whole-file inter-chunk callgraph via `--global-cfg` (v0.4.1). `--json` mode for downstream tools.|
