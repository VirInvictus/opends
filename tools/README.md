...
| [`repro`](repro/)                        | Shell + Python | 0.1.0 | DOSBox-Staging repro harness. Boots a per-bug fixture under `bugs/<id>/` with overlay-mounted writes (the install stays byte-identical), evaluates pass/fail by elapsed time + scratch-dir artifacts. v0.1.0 ships the harness pattern + one fixture (`ds1-smoke`: DS1 boots to main menu, survives 25 s). DS2 fixtures, input automation, and video capture are v0.2.0+.|
