...
use gpl_disasm::{Annotation, OPCODES, disassemble};
...
struct Cli {
    ...
    /// and write each as `<kind>-<id>.asm` under `<output>`.
...
fn main() -> Result<()> {
    ...
    if cli.all {
        ...
        for c in gff.chunks() {
            ...
            let listing = render(&disassemble(bytes));
            let name = format!(
                "{}-{}.asm",
        ...
        eprintln!(
            "disassembled {count} GPL/MAS chunks into {}",
            out_dir.display()
        );
        return Ok(());
    }
...
fn render(anns: &[Annotation]) -> String {
    let mut out = String::with_capacity(anns.len() * 48);
    for a in anns {
