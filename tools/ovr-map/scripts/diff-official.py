...
def get_map(exe_path):
    ...
    res = subprocess.run(
        [sys.executable, "tools/ovr-map/ovr-map.py", str(exe_path), "--json"],
        capture_output=True,
        text=True,
        check=True,
    )
...
def main():
    ...
    for i in range(min(len(segs_10), len(segs_110))):
        ...
        if chunk_10 == chunk_110:
            ...
            diff_count = sum(
                1 for b1, b2 in zip(chunk_10[:min_len], chunk_110[:min_len]) if b1 != b2
            )
            print(
                f"  -> {diff_count} differing bytes in shared length, plus {abs(diff_len)} bytes length difference."
            )
