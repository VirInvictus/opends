...
GHIDRA_RENAME_TEMPLATE = """\
// Generated by ovr-map %(version)s. Do not edit by hand; regenerate instead:
//   python3 tools/ovr-map/ovr-map.py <DSUN.EXE> --ghidra-rename %(syms)s -o OvrRename.java
//
// Applies the curated EXE symbol catalogue (tools/ovr-map/syms/<game>.toml)
// to an imported DSUN.EXE program: creates a function and primary label at
// each catalogue row, with the row's confidence and evidence as an end-of-
// line comment. Run AFTER OvrMap.java in the same session, so the overlay
// blocks (ovr_00, ovr_01, ...) already exist for overlay rows.
//
//@category OpenDS
import ghidra.app.script.GhidraScript;
import ghidra.program.model.address.Address;
...
public class %(classname)s extends GhidraScript {
    ...
    // type|a|b|name|confidence|evidence  rows joined by ';'
    // type R: a = file offset (resident image)
    // type O: a = overlay segment index, b = segment-local cs: offset
    private static final String ROWS = %(rows)s;
            ...
            try {
                if (f[0].equals("R")) {
                    ...
                    MemoryBlock blk = currentProgram.getMemory().getBlock(
                        String.format("ovr_%%02d", Integer.parseInt(f[1])));
                    if (blk == null) {
                        println("ovr-rename: no overlay block " + f[1]
                            + "; run OvrMap.java first");
                        return;
                    }
                    addr = blk.getStart().add(Long.parseLong(f[2]));
                }
                disassemble(addr);
                Function fn = createFunction(addr, name);
                if (fn == null) {
                    // An existing conflicting function is not fatal; still label it.
                    createLabel(addr, name, true);
                }
                setEOLComment(addr, String.format(
                    "%%s [%%s] %%s (ovr-map catalogue)", name, confidence, evidence));
                applied++;
            } catch (Exception e) {
                println("ovr-rename: row failed: " + row + " (" + e.getMessage() + ")");
                failed++;
            }
        }
        println(String.format(
            "ovr-rename: applied %%d catalogue rows, %%d failed", applied, failed));
    }
}
"""


def ghidra_rename_script(
    syms: dict[tuple[Any, int], dict[str, Any]],
    header_size: int,
    classname: str,
    syms_name: str,
) -> str:
    """Emit a Ghidra script applying a syms catalogue to the imported program.

    Same bake-data-as-strings pattern as ghidra_script(): the catalogue is
    a constant pool string, not generated statements, and no Java-side TOML
    parsing is needed.
    """

    def clean(text: str) -> str:
        # Field separators and line breaks must not appear inside a field.
        return str(text).replace("|", "/").replace(";", ",").replace("\n", " ")

    rows = []
    for (seg, off), row in sorted(
        syms.items(), key=lambda kv: (str(kv[0][0]), kv[0][1])
    ):
        kind = "R" if seg == "resident" else "O"
        a = off if seg == "resident" else seg
        b = 0 if seg == "resident" else off
        rows.append(
            f"{kind}|{a}|{b}|{clean(row['name'])}|"
            f"{clean(row.get('confidence', 'provisional'))}|"
            f"{clean(row.get('evidence', ''))}"
        )

    return GHIDRA_RENAME_TEMPLATE % {
        "version": VERSION,
        "classname": classname,
        "syms": syms_name,
        "rows": '"' + ";".join(rows) + '"',
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        metavar="SYMS",
        type=Path,
        help="emit a Ghidra script applying catalogue SYMS to the imported "
        "program (run after the --ghidra script in the same session)",
    )
    ap.add_argument(
        "--selftest",
        action="store_true",
    ...
    if args.ghidra_rename is not None:
        if not args.ghidra_rename.is_file():
            print(
                f"ovr-map: no such symbol catalogue: {args.ghidra_rename}",
                file=sys.stderr,
            )
            return 2
        try:
            catalogue = load_syms(args.ghidra_rename, report=report)
        except OvrError as e:
            print(f"ovr-map: {e}", file=sys.stderr)
            return 1
        classname = args.output.stem if args.output else "OvrRename"
        if not classname.isidentifier():
            print(
                f"ovr-map: {classname!r} is not a valid Java class name",
                file=sys.stderr,
            ...
            return 2
        script = ghidra_rename_script(
            catalogue,
            report["mz"]["header_size"],
            classname,
            args.ghidra_rename.name,
        )
        if args.output:
            args.output.write_text(script)
            print(
                f"wrote {args.output} ({len(script)} bytes, class {classname}, "
